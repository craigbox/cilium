...
if [ $? -eq 0 ]; then abort "guestbook-web policy found in cilium and they should have been cleaned" ; fi

docker exec -i ${cilium_id} cilium policy get io.cilium.k8s-policy-name=guestbook-redis 2>/dev/null

if [ $? -eq 0 ]; then abort "guestbook-redis policy found in cilium and they should have been cleaned" ; fi
